...
export default class ListView extends View {
  ...
  bindModelEvents() {
    ...
    this.pullToRefresh.on(this.pullToRefresh.EventType.SHOULD_REFRESH, this.onShouldRefresh.bind(this))
  ...
  onLoaded() {
    ...
    const listItems = this.model.shows.map(show => new ListItem(show))
  ...
  onLoadedMore(e) {
    const listEl_ = this.$('list-items')

    if (!e.diff) return

    e.diff.map(show => new ListItem(show)).forEach(c => listEl_.appendChild(c.el))

    if (e.endOfFeed) this.infiniteScroll.showEndOfList()
    else this.infiniteScroll.showSpinner()
  }
  ...
  onListItemTap(e) {
    const show = this.model.getShowById(e.targetEl.getAttribute('data-show-id'))
